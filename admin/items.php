<?php
    ...
    <?php   }else{
                ...
                echo "<div class='container'><a class='btn btn-primary btn-lg float-right' style='margin-bottom:20px' href='?do=Add'><i class='fa fa-plus'></i> Add Item</a></div> " ; 
        }elseif ($do == "Add"){?>
...
?>
                ...
                <div class="container">    
                    ...
                    <?php 
                    echo "<h2 class='text-center h1'style='margin: 60px 0px 25px;color: #675858;'>Mange [ ".$item['item_name']." ] Comments</h2>" ;
...
<?php
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                if(empty($errors)){
                    $stmt = $con -> prepare("UPDATE items SET item_name=?, item_desc=?, price=?, country=?, status=?, cat_id=?, member_id=?, item_tags=? WHERE item_id = ?") ; 
                    $stmt -> execute(array($itemName, $description, $price, $country, $status, $category, $member, $itemId, $tags)) ; 
